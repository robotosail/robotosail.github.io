...
let optionsSlippery = {
  ...
  restitution: 0.5, //the bounciness
};
...
class Initialize {
  ...
  cannonInit() {
    ...
    contactPhysics = new CANNON.ContactMaterial(
      Materials.groundMaterial,
      Materials.groundMaterial,
      options
    );
    semiContactSlippery = new CANNON.ContactMaterial(
      Materials.groundMaterial,
  ...
  threeInit() {
    ...
    // 0xffffff
    const light = new THREE.PointLight("skyblue", 0.15, 1000);
    ...
    /// FLOOR -- use box instead of plane so that player falls off the map.
    // --- cannon js
    // const groundShape = new CANNON.Plane();
    const groundShape = new CANNON.Box(
      new CANNON.Vec3(700 / 2, 700 / 2, 0.1 / 2)
    );
