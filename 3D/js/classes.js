...
import { scene, world} from "./index.js";
 ...
 class drawShape{
    constructor({update, name, shape, width = 10, height = 10, depth = 10, x, y, z, color, phong, mass, texture, wireframe, velocity ={x, y, z}}){
        this.width = width;
        this.name = name;
        this.height = height;
        this.depth = depth;
        ...
        this.mass = mass;
        if(this.shape === "box"){
            this.drawBox(this.width, this.height, this.depth, this.color, this.x, this.y, this.z);
        }
        else{
    ...
    drawBox(width, height, depth, color, x, y, z){
        ...
        this.Body = new CANNON.Body({mass: this.mass, shape: BodyShape, material: material});
